...
export default function VehicleSetupModal() {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-4">
      <div className="bg-surface border border-border rounded-3xl p-6 w-full max-w-sm max-h-[90vh] overflow-y-auto">
        <div className="text-center mb-6">
          ...
          {isConfirmMode ? (
            <>
              <h2 className="text-xl font-bold text-foreground">Confirm your vehicle</h2>
              <p className="text-muted text-sm mt-1">We need to verify your car details — please update below.</p>
            </>
          ) : (
            <>
              <h2 className="text-xl font-bold text-foreground">Welcome to vGarage</h2>
              <p className="text-muted text-sm mt-1">Tell us about your car to get started.</p>
            </>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {/* Year */}
          <div>
            <label className="block text-sm font-medium text-muted mb-1.5">Year <span className="text-accent">*</span></label>
            <select
              value={year}
          ...
          {/* Make */}
          <div>
            <label className="block text-sm font-medium text-muted mb-1.5">Make <span className="text-accent">*</span></label>
            <input
              type="text"
          ...
          {/* Model */}
          <div>
            <label className="block text-sm font-medium text-muted mb-1.5">Model <span className="text-accent">*</span></label>
            <input
              type="text"
          ...
          {/* Trim */}
          <div>
            <label className="block text-sm font-medium text-muted mb-1.5">Trim (optional)</label>
            <input
              type="text"
          ...
          <div>
            <label className="block text-sm font-medium text-muted mb-1.5">
              Current Odometer (miles) <span className="text-accent">*</span>
            </label>
            <input
              type="number"
              ...
              min="0"
              max="999999"
              className="w-full bg-surface-2 border border-border-strong rounded-xl px-4 py-3 text-foreground placeholder-faint focus:outline-none focus:border-accent/70 transition-all text-lg font-semibold"
            />
          </div>

          {/* VIN */}
          <div>
            <label className="block text-sm font-medium text-muted mb-1.5">VIN (optional)</label>
            <input
              type="text"
              ...
              maxLength={17}
              className="w-full bg-surface-2 border border-border-strong rounded-xl px-4 py-3 text-foreground placeholder-faint focus:outline-none focus:border-accent/70 transition-all font-mono text-sm"
            />
          </div>

          {/* License plate */}
          <div>
            <label className="block text-sm font-medium text-muted mb-1.5">License Plate (optional)</label>
            <input
              type="text"
          ...
          <button
            ...
            className="w-full bg-accent hover:bg-accent-hover disabled:opacity-40 disabled:cursor-not-allowed text-accent-foreground font-bold rounded-2xl px-4 py-3.5 transition-colors mt-2 shadow-lg shadow-accent/20"
          >
